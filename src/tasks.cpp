...
char gates_array   [3][11] = {"Left Gate", "CenterGate", "Right Gate"};
...
void repertory_trainning () {
  ...
  String random_repertory_gate = gates_array [random (0, 3)]; //randomize a number from 1 to 3 that corresponds to the target gates'
  ...
  if (randomized_repertory_gate == gates_array[0]) {
    open_left_gate();
  }
  else if (randomized_repertory_gate == gates_array [1]) {
    open_center_gate();
  }
  else if (randomized_repertory_gate == gates_array [2]) {
    open_right_gate();
  }
}
...
void Repeat_Repertory_trainning () {
  ...
  String randomized_gate = ExperimentConfiguration::get_current_repertory_gate();
...
void Randomize_gate_for_stages () {
